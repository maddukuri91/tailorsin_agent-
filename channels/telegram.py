...
def parse_telegram_update(update: dict[str, Any]) -> IncomingMessage | None:
    ...
    # Telegram photos / documents arrive as a ``photo`` array instead of
    # ``text``. They are no longer consumed by any agent tool (fabric
    # estimation no longer accepts a reference picture), so only the caption
    # is folded into ``text`` so it isn't lost.
    if caption:
    ...
    return IncomingMessage(
        ...
        source_user_id=(message.get("from") or {}).get("id"),
        is_start_command=(message.get("text") or "").strip() == "/start",
...
def parse_callback_query_update(update: dict[str, Any]) -> IncomingMessage | None:
    ...
    return IncomingMessage(
        user_id=chat_id,
        text=text,
        source_user_id=(callback_query.get("from") or {}).get("id"),
...
async def process_telegram_update(update: dict[str, Any]) -> dict[str, bool]:
    ...
    # A Telegram photo is delivered as a file_id rather than text and is not
    # consumed by any agent tool, so it is passed through untouched. Only the
    # caption (already folded into ``text``) is handled by the conversation.
...
async def telegram_webhook_with_path_secret(
    secret: str, update: dict[str, Any]
) -> dict[str, bool]:
